...
def generate_annotations(ii, line):

    dictLine = yaml.load(line)

    strID = dictLine['ID']
    ffsfr='/mnt/newDatasets/crowdhuman/crowdhuman_train/{}.jpg'.format(strID)
    img = cv2.imread('/mnt/newDatasets/crowdhuman/crowdhuman_train/{}.jpg'.format(strID),1)
    ...
    with open('/mnt/newDatasets/crowdhuman/crowdhuman_train/{}.txt'.format(strID), 'w+') as txtf:
...
with open("/mnt/newDatasets/crowdhuman/annotation_train.odgt") as f:
    for ii, line in tqdm(enumerate(f)):

        pcs = generate_annotations(ii, line)
